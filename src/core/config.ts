...
export async function interactiveConfig(options: {
  ...
  try {
    // 如果没有提供任何配置选项，则进入交互式配置
    const hasOptions = Object.keys(options).length > 0;

    if (!hasOptions) {
      return await runInteractiveConfig(manager);
    }
 ...
 * @returns 配置结果
 */
async function runInteractiveConfig(manager: ConfigManager): Promise<boolean> {
  ...
  const answers = await inquirer.prompt([
    {
      type: "input",
      name: "api",
      message: "请输入 Google Gemini API Key:",
      default: currentConfig.api || undefined,
    ...
    {
      type: "list",
      name: "frameExtractionStrategy",
      message: "选择帧提取策略:",
      choices: [
        { name: "单帧提取 (single) - 提取视频中间一帧", value: "single" },
        {
          name: "多帧提取 (multiple) - 提取多个均匀分布的帧",
          value: "multiple",
        },
        { name: "关键帧提取 (keyframes) - 提取视频关键帧", value: "keyframes" },
      ],
    ...
    },
    {
      type: "number",
      name: "batchSize",
      message: "设置批量处理大小 (1-100):",
      default: currentConfig.batchProcessing.batchSize,
      validate: (input: number) => {
        if (!Number.isInteger(input) || input < 1 || input > 100) {
          return "批量处理大小必须是 1-100 之间的整数";
        }
        return true;
      },
    },
    {
      type: "number",
      name: "filenameLength",
      message: "设置文件名长度限制 (5-50):",
      default: currentConfig.promptConfig.filenameLength,
      validate: (input: number) => {
        if (!Number.isInteger(input) || input < 5 || input > 50) {
          return "文件名长度限制必须是 5-50 之间的整数";
        }
        return true;
      },
    },
    {
      ...
      message: "是否要设置自定义 AI 分析提示模板?",
      default: !!currentConfig.promptConfig.customTemplate,
    },
    {
      type: "input",
      name: "customPrompt",
      message: "请输入自定义 AI 分析提示模板:",
      default: currentConfig.promptConfig.customTemplate || "",
      when: (answers: { useCustomPrompt: boolean }) => answers.useCustomPrompt,
      validate: (input: string) => {
        if (input?.trim()) {
          // 检查是否包含不允许的内容
          const restrictedPatterns = [
          ...
          for (const pattern of restrictedPatterns) {
            if (pattern.test(input)) {
              return "自定义模板不能包含 JSON 格式要求部分，该部分由系统自动添加";
            }
          }
        }
        return true;
    ...
    },
    {
      ...
      message: "选择图像处理质量:",
      choices: [
        { name: "高质量 (90)", value: 90 },
        { name: "标准质量 (75)", value: 75 },
        { name: "低质量 (60)", value: 60 },
      ],
      default: currentConfig.imageProcessing.quality,
    },
    {
      type: "confirm",
      name: "verbose",
      message: "是否启用详细输出和调试模式?",
  ...
  try {
    ...
    manager.setBatchProcessingConfig({ batchSize: answers.batchSize });
    manager.setPromptConfig({
      filenameLength: answers.filenameLength,
      customTemplate: answers.useCustomPrompt
        ? answers.customPrompt
        : undefined,
    });
    manager.setImageProcessingConfig({ quality: answers.imageQuality });
    manager.setVerboseMode(answers.verbose);
